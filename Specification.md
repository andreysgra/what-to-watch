# Техническое задание к проекту «Что посмотреть»
...
Если пользователь не авторизован, то при переходе к этим страницам выполняется перенаправление на страницу «Sign In» (`/login`).
...
Сервер доступен по адресу: `https://16.design.htmlacademy.pro/wtw`.

Спецификация по взаимодействию с сервером в формате [OpenAPI](https://16.design.htmlacademy.pro/spec/wtw).
